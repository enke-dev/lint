...
suite('biome', () => {
  describe('naive check that biome grabs issues', () => {
    ...
    it('finds issues in HTML', async () => {
      const { errorCount } = await runBiomeOnFile('test/test.html');
  ...
  describe('naive check that biome can fix issues', () => {
    it('applies fixable issues in Typescript', async () => {
      const { errorCount: beforeCount } = await runBiomeOnFile('test/test.ts');
      const { errorCount: afterCount } = await runBiomeOnFile('test/test.ts', true);
      ok(
    ...
    it('applies fixable issues in JSON', async () => {
      const { errorCount: beforeCount } = await runBiomeOnFile('test/test.json');
      const { errorCount: afterCount } = await runBiomeOnFile('test/test.json', true);
      ok(
        afterCount <= beforeCount,
